...
mov AX, 0xB800 ; Set AX to the segment address of VGA text mode memory.
mov ES, AX     ; Set ES to the VGA text mode memory segment.
mov DI, 8*160  ; Set DI to the offset for row 8 (each row is 160 bytes: 80 characters * 2 bytes per character).
mov AL, 'K'    ; Load the character 'K' into AL.
mov AH, 0x07   ; Load the standard color attribute (gray on black) into AH.
stosw          ; Store the word in AX (AL and AH) to the address pointed by ES:DI and increment DI by 2.

; Data segment: Define the characters and strings used in the above methods.
