...
handle_backspace:
    ...
    mov ah, 0x03
    xor bh, bh                ; Clear BH (page number)
    int 0x10                  ; BIOS interrupt to get cursor position
    ; Cursor row in DH, cursor column in DL

    ; Check if we are at the start of a line
    cmp dl, 0
    jne move_back_one         ; If not at the start, just move back one position

    ; Move to the previous line if at the start of a line
    ...
    ; Back up in the video buffer and clear the character
    push ax                   ; Save AX
    mov ax, 0xB800            ; Segment address for color text video buffer
    mov es, ax                ; Set ES to video buffer
    mov ax, dh
    shl ax, 1                 ; Multiply row by 2 (each row is two bytes in video memory)
    shl ax, 5                 ; Multiply by 32 to get the correct offset (80 columns per row)
    mov di, ax                ; DI = row offset in video memory
    add di, dl                ; Add the column offset
    mov byte [es:di], ' '     ; Clear the character on screen
    pop ax                    ; Restore AX

    jmp read_key              ; Jump back to read the next key

move_back_one:
    ; Move back one position on the same line
    dec dl
    ; Existing logic to move back one position...
    ; (Make sure to adjust the video buffer position as well)
...
H;andling Backspace: When the Backspace key is pressed, the program checks if the cursor is at the start of the buffer. If not, it moves the cursor back, erases the character from the buffer, and calls a subroutine to handle the cursor movement on the screen.
