...
public final class Library extends Root {
    ...
    private CostumerInterface currentCustomer;
    ...
    public void supplyStock ( BookInterface product ) {
    ...
    public int checkStock ( BookInterface product , int amount ) {
        ...
        if (stock.get (productID) < amount) {
            System.out.println ("*   Stock is less than needed amount of " + amount + " items. \n The library will sell only " + stock.get (productID) + " items of this product");
            return stock.get (productID);
        }
        return amount;
    }

    public void removeStock ( BookInterface product , int amount ) {
        this.productID = product.getId ( );
    ...
    public void setLibrarian ( OrderChecker orderPresent ) {
        this.librarian = orderPresent;
    }
    ...
    public void setCurrentCustomer ( CostumerInterface customer ) {
    ...
    public void addBookToOrder ( BookInterface product , int amount ) {
        System.out.println (" ");
        if (product.getPrescriptionNeeded ( )) {
            if ((int) (Math.random ( ) * 100) > 40) {
                this.currentCustomer.addBook (product.getId ( ));
            }
            this.checkOrder (this.currentCustomer.hasBook (product.getId ( )));
            if (!this.currentCustomer.hasBook (product.getId ( ))) {
                return;
            }
        }
    ...
    public void isRobbed ( ) {
        ...
        System.out.println ("Total loss for library is ..." + this.salesOfCurrentDay + " lei");
