...
async def handle_non_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handle messages that are not plain text (e.g., images, videos, or files).
    """
    logger.info(f"Received non-text message from {update.effective_user.username}")
    await update.message.reply_text("Sorry, I can only process text messages. No images, videos, or files are allowed.")
...
def main():
    ...
    # Add handler for non-text messages (images, videos, files, etc.)
    non_text_handler = MessageHandler(
        ~filters.TEXT,  # Matches anything that is not plain text
